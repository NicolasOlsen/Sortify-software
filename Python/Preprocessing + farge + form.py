...
def is_ball_shape(contour):
    """
    Returns True if the contour is shaped like a circle/ball.
    We do a polygon approximation and check if the 
    contour has enough 'sides' to be considered a circle.
    """
    ...
    # If the approximated contour has many vertices, it's closer to a circle.
    # Tune the threshold as needed; ~6+ sides often indicates near-circular.
    if len(approx) > 6:
        return True
    return False

def detect_and_label(mask, color_name, frame, color):
    """Detects and labels multiple circles (now verifying shape) and removes stale detections."""
    global prev_circles
    ...
    for contour in contours:
        ...
        if 800 < area < 45000:  # Adjusted size restrictions
            ...
            # Existing “circularity” gate (coarse check):
            if 0.6 < circularity < 1.2:
                # Additional shape check to confirm it's truly a "ball"
                if is_ball_shape(contour):
                    # Accept it as a ball
                    detected_circles.append((int(x), int(y), int(radius)))
                # else: skip it, not a ball shape
            # else: skip if not in the circularity range

    # If no detections, clear old data (fixes "stuck circles" issue)
    if detected_circles:
        prev_circles[color_name] = detected_circles
    else:
        prev_circles[color_name] = []

    # Draw only currently detected circles
    for x, y, radius in prev_circles[color_name]:
        cv2.circle(frame, (x, y), radius, color, 2)
        # Show "Ball" to highlight that it passed shape check
